  ...
  .createServer(async function (req, res) {
    switch (req.url) {
      case "/api/proveedores":
        ...
        llenarTablaProveedor().then(_=>fs.readFile('./project/indexModificado.html', function (error, content) {
          if (error) {
             res.writeHead(500);
             res.end('Error');
          } else {
      ...
      case "/api/clientes":
        ...
        llenarTablaCliente().then(_=> fs.readFile('./project/indexModificado.html', function (error, content) {
          if (error) {
             res.writeHead(500);
             res.end('Error');
          } else {
...
const promiseProveedores = async () => {
 ...
 async function llenarTablaCliente(){ promiseClientes().then((data)=>{
  let tabla = "";
  for (i in data){
    ...
    let aux = "<tr> " + "<td> " + idcliente +" </td> " + "<td> " + nombreCompania +" </td> " + "<td> " + nombreContacto +" </td> " + " </tr>"
    tabla += aux
  }
  fs.readFile("./project/index.html","utf-8", function (err,data)
  {
    ...
    fs.writeFile("./project/indexModificado.html",modificado3,"utf8",function(err){
...
async function llenarTablaProveedor(){ promiseProveedores().then((data)=>{
  let tabla = "";
  for (i in data){
  ...
  fs.readFile("./project/index.html","utf-8", function (err,data)
  {
    ...
    var modificado = data.replace(/auxiliarFSNOD/g,tabla)
    var modificado2 = modificado.replace(/identificador/g,"Id del proveedor")
